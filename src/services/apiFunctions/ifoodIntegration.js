...
async function ifoodIntegration(chosenData) {
  try {
    const ifoodToPost = chosenData.storeCode.map(ifoodData => new Ifood({
      _cod_store: ifoodData.storeCode,
      _desc_store_partner: ifoodData.storeName
    }))

    await Promise.all(ifoodToPost.map(ifoodItem => 
      postToSaipos(ifoodItem, `${API_BASE_URL}/stores/${storeId}/partners_sale/verify_partner_sale_api_login`)
    ))
