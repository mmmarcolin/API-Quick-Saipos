...
export async function handleWorkers(quantity, dailyRate, worker, extra) {
    ...
    if (quantity) {
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            results.push({ desc, dailyRate: rate });
        };
    }
    
    // Add extra workers if specified
    worker === "Garçom" && extra ? results.push({ desc: "Caixa", dailyRate: 0 }) : null
    worker === "Entregador" && extra ? results.push({ desc: "Entrega fácil", dailyRate: 0 }) : null
    
    return results.length > 0 ? results : "" // Return
}